<div class="mb-6">
    <label for="photos" class="label">Photo</label>
        @if (isset($comic->photos))
            <div class="flex gap-2">
                @foreach ($comic->photos as $photo)
                    <img class="rounded-t-lg  aspect-square !w-20 pb-2" alt="cover"
                    src="{{ 
        ...
        @endif
        <input class="input" type="file" name="photos[]" id="photos" value={{ $comic->photos ?? old('photos') }} required multiple >
        @error('photos[]')
